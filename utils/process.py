...
def get_country_counts():
	tcountries = defaultdict(list)
		...
		pi = PeerInfo.objects.filter(peerrecord__session__torrent__id=t.id).distinct().annotate(countrycount=Count('country')).order_by('-countrycount')
		for p in pi:
...
def get_torrent_stats(torrent, countries=None, asns=None):
    ...
    total_ip = PeerRecord.objects.filter(session__torrent__id=torrent.id).values_list('ip', flat=True).distinct().count()
