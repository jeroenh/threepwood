...
def get_country_counts(version = "1204"):
        ...
        for t in Torrent.objects.all()[6:]:
                ...
                pi = PeerInfo.objects.filter(peerrecord__session__torrent__id=t.id, peerrecord__session__version =version).distinct().annotate(countrycount=Count('country')).order_by('-countrycount')
                for p in pi:
...
#    kpn_as = [5615, 49562, 286, 1134, 12469]    
    ...
    total_ip = PeerRecord.objects.filter(session__version=version, session__torrent__id=torrent.id).values_list('ip', flat=True).distinct().count()
    if not total_ip:
		return 

    for country_code in countries:
	
    	others = {'total_unique_ips':0,'percent_of_country':0,'percent_of_total':0, 'name':'others', 'country':country_code }
#   	kpn_stats = {'total_unique_ips':0,'percent_of_country':0,'percent_of_total':0, 'name':'PT Koninklijke KPN N.V.', 'country':country_code, 'asn':[5615, 49562, 286, 1134, 12469]}
    	kpn_stats = {'total_unique_ips':0,'percent_of_country':0,'percent_of_total':0, 'name':'PT Koninklijke KPN N.V.', 'country':country_code, 'asn':[5615, 49562, 286]}

        #gets all the unique ips for this torrent that come from this country
        total_ip_country = PeerRecord.objects.filter(session__torrent__id=torrent.id,peerinfo__country=country_code).values_list('ip',
