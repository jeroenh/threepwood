...
def get_country_stats_new():
    ...
    f = open("countries-%s.txt" % (version), 'w')
    ...
    for t in Torrent.objects.all()[6:]:
        ...
        for country_code in countries:
            ...
            torrent_stats_raw[t].append(PeerRecord.objects.filter(session__torrent__id=t.id,session__version=version,
                                                                      peerinfo__country=country_code)
            .values_list('ip', flat=True).distinct().count())

        #count total ips for this torrent
        total_ip_count = PeerRecord.objects.filter(session__torrent__id=t.id, session__version=version).values_list('ip', flat=True).distinct().count()
        ...
        for as_number in country_as:
            if as_number in [5615, 49562, 286, 1134, 12469, 8737]:
                total_kpn += PeerRecord.objects.filter(session__torrent__id=t.id,
                                                       peerinfo__asnumber=as_number, session__version=version,peerinfo__country=country_code).distinct().values_list('ip',
                                                                                                 flat=True).distinct().count()
            else:
                totals[t].append(PeerRecord.objects.filter(session__torrent__id=t.id, peerinfo__asnumber=as_number, session__version=version,peerinfo__country=country_code).distinct().values_list('ip',
                                                                                           flat=True).distinct().count())
